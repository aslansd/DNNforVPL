...
from make_manifold_data import make_manifold_data
...
# The DNN model for VPL Attribution
class DNNforVPL_Attribution(nn.Module):
    
    def __init__(self, num_classes = 1):
        
        super(DNNforVPL_Attribution, self).__init__()
...
def main():
    ...
    os.mkdir('New_Results_AlexNet_Captum_Manifold')
    
    for num_simulation in range(0, 1):
        print('Simulation:   ', num_simulation + 1)
        
        os.mkdir('New_Results_AlexNet_Captum_Manifold/Simulation_' + str(num_simulation + 1))
        ...
        for group_training in ['group1', 'group2', 'group3', 'group4']:
            ...
            os.mkdir('New_Results_AlexNet_Captum_Manifold/Simulation_' + str(num_simulation + 1) + '/' + group_training)
            ...
            for layer_freeze in [None]: # [0, 3, 6, 8, 10]
                ...
                if layer_freeze == None:
                    os.mkdir('New_Results_AlexNet_Captum_Manifold/Simulation_' + str(num_simulation + 1) + '/' + group_training + '/before_training')
                    saving_folder = 'New_Results_AlexNet_Captum_Manifold/Simulation_' + str(num_simulation + 1) + '/' + group_training + '/before_training'
                ...
                os.mkdir('New_Results_AlexNet_Captum_Manifold/Simulation_' + str(num_simulation + 1) + '/' + group_training + '/after_training_' + str(layer_freeze))
                saving_folder = 'New_Results_AlexNet_Captum_Manifold/Simulation_' + str(num_simulation + 1) + '/' + group_training + '/after_training_' + str(layer_freeze)
                ...
                # Load the PyTorch model for Attribution
                model_attribution = DNNforVPL_Attribution()
                
                model_dict = model.state_dict()
                model_attribution_dict = model_attribution.state_dict()
                
                # Overwrite entries in the existing state dict
                model_attribution_dict.update(model_dict)
                # Load the new state dict
                model_attribution.load_state_dict(model_attribution_dict)
                
                # Send the model to GPU/CPU
                model_attribution = model_attribution.to(device)
                
                # feature_sample_artiphysiology = np.zeros((num_sample_artiphysiology, 3), dtype = np.int64)
                
                # all_unit_conductance_Conv2d_1 = np.zeros((num_sample_artiphysiology, 64, 55, 55), dtype = np.float32)
                # all_unit_conductance_Conv2d_2 = np.zeros((num_sample_artiphysiology, 192, 27, 27), dtype = np.float32)
                # all_unit_conductance_Conv2d_3 = np.zeros((num_sample_artiphysiology, 384, 13, 13), dtype = np.float32)
                # all_unit_conductance_Conv2d_4 = np.zeros((num_sample_artiphysiology, 256, 13, 13), dtype = np.float32)
                # all_unit_conductance_Conv2d_5 = np.zeros((num_sample_artiphysiology, 256, 13, 13), dtype = np.float32)
                
                # conductance_layer_conv_1 = LayerConductance(model_attribution, model_attribution.features[0])
                # conductance_layer_conv_2 = LayerConductance(model_attribution, model_attribution.features[3])
                # conductance_layer_conv_3 = LayerConductance(model_attribution, model_attribution.features[6])
                # conductance_layer_conv_4 = LayerConductance(model_attribution, model_attribution.features[8])
                # conductance_layer_conv_5 = LayerConductance(model_attribution, model_attribution.features[10])
                ...
                #     all_unit_conductance_Conv2d_1[i] = conductance_layer_conv_1.attribute(x_sample, baselines = x_baseline).detach().cpu().clone().numpy()
                #     all_unit_conductance_Conv2d_2[i] = conductance_layer_conv_2.attribute(x_sample, baselines = x_baseline).detach().cpu().clone().numpy()
                #     all_unit_conductance_Conv2d_3[i] = conductance_layer_conv_3.attribute(x_sample, baselines = x_baseline).detach().cpu().clone().numpy()
                #     all_unit_conductance_Conv2d_4[i] = conductance_layer_conv_4.attribute(x_sample, baselines = x_baseline).detach().cpu().clone().numpy()
                #     all_unit_conductance_Conv2d_5[i] = conductance_layer_conv_5.attribute(x_sample, baselines = x_baseline).detach().cpu().clone().numpy()
                                        
                # scipy.io.savemat(saving_folder + '/feature_sample_artiphysiology.mat', mdict = {'feature_sample_artiphysiology': feature_sample_artiphysiology})
                
                # scipy.io.savemat(saving_folder + '/all_unit_conductance_Conv2d_1.mat', mdict = {'all_unit_conductance_Conv2d_1': all_unit_conductance_Conv2d_1})
                # scipy.io.savemat(saving_folder + '/all_unit_conductance_Conv2d_2.mat', mdict = {'all_unit_conductance_Conv2d_2': all_unit_conductance_Conv2d_2})
                # scipy.io.savemat(saving_folder + '/all_unit_conductance_Conv2d_3.mat', mdict = {'all_unit_conductance_Conv2d_3': all_unit_conductance_Conv2d_3})
                # scipy.io.savemat(saving_folder + '/all_unit_conductance_Conv2d_4.mat', mdict = {'all_unit_conductance_Conv2d_4': all_unit_conductance_Conv2d_4})
                # scipy.io.savemat(saving_folder + '/all_unit_conductance_Conv2d_5.mat', mdict = {'all_unit_conductance_Conv2d_5': all_unit_conductance_Conv2d_5})
                              
                # # # Boxplotting the channel conductance of three features of convolutional layer
                # # SF_box_channel_conductance_Conv2d_1 = []
                # # SF_box_channel_conductance_Conv2d_2 = []
                # # SF_box_channel_conductance_Conv2d_3 = []
                # # SF_box_channel_conductance_Conv2d_4 = []
                # # SF_box_channel_conductance_Conv2d_5 = []
                
                # # for i in range(0, len(SF_tuning)):                        
                # #     SF_box_channel_conductance_Conv2d_1.append(np.mean(all_channel_conductance_Conv2d_1[feature_sample_artiphysiology[:, 0] == SF_tuning[i], :], axis = 1))
                # #     SF_box_channel_conductance_Conv2d_2.append(np.mean(all_channel_conductance_Conv2d_2[feature_sample_artiphysiology[:, 0] == SF_tuning[i], :], axis = 1))
                # #     SF_box_channel_conductance_Conv2d_3.append(np.mean(all_channel_conductance_Conv2d_3[feature_sample_artiphysiology[:, 0] == SF_tuning[i], :], axis = 1))
                # #     SF_box_channel_conductance_Conv2d_4.append(np.mean(all_channel_conductance_Conv2d_4[feature_sample_artiphysiology[:, 0] == SF_tuning[i], :], axis = 1))
                # #     SF_box_channel_conductance_Conv2d_5.append(np.mean(all_channel_conductance_Conv2d_5[feature_sample_artiphysiology[:, 0] == SF_tuning[i], :], axis = 1))
                    
                # # Ori_box_channel_conductance_Conv2d_1 = []
                # # Ori_box_channel_conductance_Conv2d_2 = []
                # # Ori_box_channel_conductance_Conv2d_3 = []
                # # Ori_box_channel_conductance_Conv2d_4 = []
                # # Ori_box_channel_conductance_Conv2d_5 = []
                
                # # for i in range(0, len(Ori_tuning)):
                # #     Ori_box_channel_conductance_Conv2d_1.append(np.mean(all_channel_conductance_Conv2d_1[feature_sample_artiphysiology[:, 1] == Ori_tuning[i], :], axis = 1))
                # #     Ori_box_channel_conductance_Conv2d_2.append(np.mean(all_channel_conductance_Conv2d_2[feature_sample_artiphysiology[:, 1] == Ori_tuning[i], :], axis = 1))
                # #     Ori_box_channel_conductance_Conv2d_3.append(np.mean(all_channel_conductance_Conv2d_3[feature_sample_artiphysiology[:, 1] == Ori_tuning[i], :], axis = 1))
                # #     Ori_box_channel_conductance_Conv2d_4.append(np.mean(all_channel_conductance_Conv2d_4[feature_sample_artiphysiology[:, 1] == Ori_tuning[i], :], axis = 1))
                # #     Ori_box_channel_conductance_Conv2d_5.append(np.mean(all_channel_conductance_Conv2d_5[feature_sample_artiphysiology[:, 1] == Ori_tuning[i], :], axis = 1))
                
                # # Phase_box_channel_conductance_Conv2d_1 = []
                # # Phase_box_channel_conductance_Conv2d_2 = []
                # # Phase_box_channel_conductance_Conv2d_3 = []
                # # Phase_box_channel_conductance_Conv2d_4 = []
                # # Phase_box_channel_conductance_Conv2d_5 = []
                
                # # for i in range(0, 360):
                # #     Phase_box_channel_conductance_Conv2d_1.append(np.mean(all_channel_conductance_Conv2d_1[feature_sample_artiphysiology[:, 2] == i + 1, :], axis = 1))
                # #     Phase_box_channel_conductance_Conv2d_2.append(np.mean(all_channel_conductance_Conv2d_2[feature_sample_artiphysiology[:, 2] == i + 1, :], axis = 1))
                # #     Phase_box_channel_conductance_Conv2d_3.append(np.mean(all_channel_conductance_Conv2d_3[feature_sample_artiphysiology[:, 2] == i + 1, :], axis = 1))
                # #     Phase_box_channel_conductance_Conv2d_4.append(np.mean(all_channel_conductance_Conv2d_4[feature_sample_artiphysiology[:, 2] == i + 1, :], axis = 1))
                # #     Phase_box_channel_conductance_Conv2d_5.append(np.mean(all_channel_conductance_Conv2d_5[feature_sample_artiphysiology[:, 2] == i + 1, :], axis = 1))
                    
                # # for feature in ['SF', 'Ori', 'Phase']:
                # #     for conv_layer_num in [1, 2, 3, 4, 5]:
                # #         plt.figure()
                # #         plt.title("%s Boxplot Channel conductance of the Convolutional Layer %d" % (feature, conv_layer_num))
                # #         plt.xlabel(feature)
                # #         plt.ylabel("Channel conductance")
                # #         variable_name = feature + '_box_channel_conductance_Conv2d_' + str(conv_layer_num)
                # #         plt.boxplot(vars()[variable_name])
                # #         plt.show()
                # #         plt.savefig(saving_folder + '/' + feature + ' Boxplot Channel conductance of the Convolutional Layer ' + str(conv_layer_num) + '.tif')
                # #         plt.close()
                
                # ### Ablation Studies for Outlier Channels Based on Conductance Values
                
                # mean_channel_conductance_Conv2d_1 = np.mean(all_unit_conductance_Conv2d_1, axis = (2, 3))
                # mean_channel_conductance_Conv2d_2 = np.mean(all_unit_conductance_Conv2d_2, axis = (2, 3))
                # mean_channel_conductance_Conv2d_3 = np.mean(all_unit_conductance_Conv2d_3, axis = (2, 3))
                # mean_channel_conductance_Conv2d_4 = np.mean(all_unit_conductance_Conv2d_4, axis = (2, 3))
                # mean_channel_conductance_Conv2d_5 = np.mean(all_unit_conductance_Conv2d_5, axis = (2, 3))
                
                # abs_mean_channel_conductance_Conv2d_1 = np.abs(mean_channel_conductance_Conv2d_1)
                # abs_mean_channel_conductance_Conv2d_2 = np.abs(mean_channel_conductance_Conv2d_2)
                # abs_mean_channel_conductance_Conv2d_3 = np.abs(mean_channel_conductance_Conv2d_3)
                # abs_mean_channel_conductance_Conv2d_4 = np.abs(mean_channel_conductance_Conv2d_4)
                # abs_mean_channel_conductance_Conv2d_5 = np.abs(mean_channel_conductance_Conv2d_5)
                ...
                #         modified_z_score[np.isnan(modified_z_score)] = -np.inf
                ...
                #         model_output = model(x_sample, x_baseline).squeeze().detach().cpu().clone().numpy()
                ...
                #         abs_mean_variable_name = 'abs_mean_channel_conductance_Conv2d_' + str(j + 1)                                                
                ...
                model_attribution = model_attribution.to(device)
                model_attribution = model_attribution.eval()
    
                sampled_classes = 2
                examples_per_class = 100
                
                x_tensor_train_dataset = []
                y_tensor_train_dataset = []
                                
                if group_training == 'group1' or group_training == 'group3':
                    for i in range(0, int(sampled_classes * examples_per_class / len(Ori_training))):
                        for j in range(0, len(Ori_training)):
                            z_val_shuffle = copy.deepcopy(z_val_training)
                            random.shuffle(z_val_shuffle[0, j, :])                      
                            x_tensor_train_dataset.append(torch.index_select(x_tensor_training, 0, torch.tensor(z_val_shuffle[0, j, 0], dtype = torch.long)))
                            y_tensor_train_dataset.append(torch.index_select(y_tensor_training, 0, torch.tensor(z_val_shuffle[0, j, 0], dtype = torch.long)))
                
                elif group_training == 'group2' or group_training == 'group4':
                    for i in range(0, int(sampled_classes * examples_per_class / len(Ori_training))):
                        for j in range(0, len(Ori_training)):
                            z_val_shuffle = copy.deepcopy(z_val_training)
                            random.shuffle(z_val_shuffle[1, j, :])                      
                            x_tensor_train_dataset.append(torch.index_select(x_tensor_training, 0, torch.tensor(z_val_shuffle[1, j, 0], dtype = torch.long)))
                            y_tensor_train_dataset.append(torch.index_select(y_tensor_training, 0, torch.tensor(z_val_shuffle[1, j, 0], dtype = torch.long)))
                            
                x_tensor_train_dataset = torch.stack(x_tensor_train_dataset)
                y_tensor_train_dataset = torch.stack(y_tensor_train_dataset)
                
                train_dataset = []
                train_dataset.append(x_tensor_train_dataset.squeeze())
                train_dataset.append(y_tensor_train_dataset.squeeze())
                
                data = make_manifold_data(train_dataset, sampled_classes, examples_per_class, seed = 0)
                data = [d.to(device) for d in data]
                
                activations = extractor(model_attribution, data, layer_types = ['Conv2d'])
                ...
                plt.tight_layout()
                plt.show()
                plt.savefig(saving_folder + '/Neural Manifolds Analysis_Training Stimuli.tif')
                plt.close()
                ...
                x_tensor_transfer_dataset = []
                y_tensor_transfer_dataset = []
                                
                for i in range(0, int(sampled_classes * examples_per_class / len(Ori_transfer))):
                    for j in range(0, len(Ori_transfer)):
                        z_val_shuffle = copy.deepcopy(z_val_transfer)
                        random.shuffle(z_val_shuffle[0, j, :])                      
                        x_tensor_transfer_dataset.append(torch.index_select(x_tensor_transfer, 0, torch.tensor(z_val_shuffle[0, j, 0], dtype = torch.long)))
                        y_tensor_transfer_dataset.append(torch.index_select(y_tensor_transfer, 0, torch.tensor(z_val_shuffle[0, j, 0], dtype = torch.long)))
                        
                x_tensor_transfer_dataset = torch.stack(x_tensor_transfer_dataset)
                y_tensor_transfer_dataset = torch.stack(y_tensor_transfer_dataset)
                
                test_dataset = []
                test_dataset.append(x_tensor_transfer_dataset.squeeze())
                test_dataset.append(y_tensor_transfer_dataset.squeeze())
                
                model_attribution = model_attribution.eval()
                results = analyze(model_attribution, test_dataset, sampled_classes = sampled_classes, examples_per_class = examples_per_class, layer_types = layer_types, projection = projection, projection_dimension = projection_dimension, seed = seed)
                ...
                plt.tight_layout()
                plt.show()
                plt.savefig(saving_folder + '/Neural Manifolds Analysis_Transfer Stimuli.tif')
                plt.close()
