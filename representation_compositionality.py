...
def main():
    ...
    for simulation_counter in range(number_simulation):
        ...
        with torch.set_grad_enabled(False):
            for i, (x_val, y_val) in enumerate(loader):
                ...
                for j in range(batch_size):
                    x_val_sample = torch.index_select(x_val, 0, j)
                    
                    x_val_sample_channel_shuffled = []
                                                
                    idx = torch.randperm(torch.index_select(x_val_sample, 0, 0).nelement())
                    
                    for k in range(3):
                        x_val_sample_channel_shuffled.append(torch.index_select(x_val_sample, 0, k).view(-1)[idx].view(torch.index_select(x_val_sample, 0, k).size()))
                        
                    x_val_sample_channel_shuffled = torch.stack(x_val_sample_channel_shuffled)
                    x_val_shuffled.append(x_val_sample_channel_shuffled)
                    
                x_val_shuffled = torch.stack(x_val_shuffled)
                
                all_simulation_compositionality[simulation_counter, 0] = torch.pow(torch.mean(torch.pow(x_val_shuffled - x_val, 2)), 0.5).item()
                ...
                all_simulation_compositionality[simulation_counter, 1] = torch.pow(torch.mean(torch.pow(unit_activity_layer_2_0.detach().cpu().clone().numpy().reshape(batch_size, -1) - unit_activity_layer_1_0.detach().cpu().clone().numpy().reshape(batch_size, -1), 2)), 0.5).item()
                all_simulation_compositionality[simulation_counter, 2] = torch.pow(torch.mean(torch.pow(unit_activity_layer_2_3.detach().cpu().clone().numpy().reshape(batch_size, -1) - unit_activity_layer_1_3.detach().cpu().clone().numpy().reshape(batch_size, -1), 2)), 0.5).item()
                all_simulation_compositionality[simulation_counter, 3] = torch.pow(torch.mean(torch.pow(unit_activity_layer_2_6.detach().cpu().clone().numpy().reshape(batch_size, -1) - unit_activity_layer_1_6.detach().cpu().clone().numpy().reshape(batch_size, -1), 2)), 0.5).item()
                all_simulation_compositionality[simulation_counter, 4] = torch.pow(torch.mean(torch.pow(unit_activity_layer_2_8.detach().cpu().clone().numpy().reshape(batch_size, -1) - unit_activity_layer_1_8.detach().cpu().clone().numpy().reshape(batch_size, -1), 2)), 0.5).item()
                all_simulation_compositionality[simulation_counter, 5] = torch.pow(torch.mean(torch.pow(unit_activity_layer_2_10.detach().cpu().clone().numpy().reshape(batch_size, -1) - unit_activity_layer_1_10.detach().cpu().clone().numpy().reshape(batch_size, -1), 2)), 0.5).item()
        ...
        # Filter out unnecessary keys
        pretrained_dict_model = {k : v for k, v in pretrained_dict.items() if k in model_dict_1}
        # Overwrite entries in the existing state dict
        model_dict_1.update(pretrained_dict_model)
        # Load the new state dict
        ...
        # Filter out unnecessary keys
        pretrained_dict_model = {k : v for k, v in pretrained_dict.items() if k in model_dict_2}
        # Overwrite entries in the existing state dict
        model_dict_2.update(pretrained_dict_model)
        # Load the new state dict
        ...
        # Filter out unnecessary keys
        pretrained_dict_model = {k : v for k, v in pretrained_dict.items() if k in model_dict_3}
        # Overwrite entries in the existing state dict
        model_dict_3.update(pretrained_dict_model)
        # Load the new state dict
        ...
        # Filter out unnecessary keys
        pretrained_dict_model = {k : v for k, v in pretrained_dict.items() if k in model_dict_4}
        # Overwrite entries in the existing state dict
        model_dict_4.update(pretrained_dict_model)
        # Load the new state dict
        model_4.load_state_dict(model_dict_4)
        
        model_5 = AlexNet_5()
        model_dict_5 = model_5.state_dict()
        
        # Filter out unnecessary keys
        pretrained_dict_model = {k : v for k, v in pretrained_dict.items() if k in model_dict_5}
        # Overwrite entries in the existing state dict
        model_dict_5.update(pretrained_dict_model)
        # Load the new state dict
        model_5.load_state_dict(model_dict_5)
        ...
        with torch.set_grad_enabled(False):
            for i, (x_val, y_val) in enumerate(loader):
                ...
                for j in range(batch_size):
                    x_val_sample_0 = torch.index_select(unit_activity_layer_0, 0, j)
                    x_val_sample_3 = torch.index_select(unit_activity_layer_3, 0, j)
                    x_val_sample_6 = torch.index_select(unit_activity_layer_6, 0, j)
                    x_val_sample_8 = torch.index_select(unit_activity_layer_8, 0, j)
                    x_val_sample_10 = torch.index_select(unit_activity_layer_10, 0, j)
                    ...
                    idx_0 = torch.randperm(torch.index_select(x_val_sample_0, 0, 0).nelement())
                    idx_3 = torch.randperm(torch.index_select(x_val_sample_3, 0, 0).nelement())
                    idx_6 = torch.randperm(torch.index_select(x_val_sample_6, 0, 0).nelement())
                    idx_8 = torch.randperm(torch.index_select(x_val_sample_8, 0, 0).nelement())
                    idx_10 = torch.randperm(torch.index_select(x_val_sample_10, 0, 0).nelement())
                    
                    for k in range(64):
                        x_val_sample_channel_shuffled_0.append(torch.index_select(x_val_sample_0, 0, k).view(-1)[idx_0].view(torch.index_select(x_val_sample_0, 0, k).size()))
                        
                    for k in range(192):
                        x_val_sample_channel_shuffled_3.append(torch.index_select(x_val_sample_3, 0, k).view(-1)[idx_3].view(torch.index_select(x_val_sample_3, 0, k).size()))
                        
                    for k in range(384):
                        x_val_sample_channel_shuffled_6.append(torch.index_select(x_val_sample_6, 0, k).view(-1)[idx_6].view(torch.index_select(x_val_sample_6, 0, k).size()))
                        
                    for k in range(256):
                        x_val_sample_channel_shuffled_8.append(torch.index_select(x_val_sample_8, 0, k).view(-1)[idx_8].view(torch.index_select(x_val_sample_8, 0, k).size()))
                        
                    for k in range(256):
                        x_val_sample_channel_shuffled_10.append(torch.index_select(x_val_sample_10, 0, k).view(-1)[idx_10].view(torch.index_select(x_val_sample_10, 0, k).size()))
                        
                    x_val_sample_channel_shuffled_0 = torch.stack(x_val_sample_channel_shuffled_0)
                    x_val_sample_channel_shuffled_3 = torch.stack(x_val_sample_channel_shuffled_3)
                    x_val_sample_channel_shuffled_6 = torch.stack(x_val_sample_channel_shuffled_6)
                    x_val_sample_channel_shuffled_8 = torch.stack(x_val_sample_channel_shuffled_8)
                    x_val_sample_channel_shuffled_10 = torch.stack(x_val_sample_channel_shuffled_10)
