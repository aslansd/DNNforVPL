"""
Adapted from: https://github.com/mrtnoshad/EDGE
"""
...
"""
    ...
    Arguments:   
    X: N * d_x and Y is N * d_Y data sets.
    U (optional): An upper bound on the MI. It doesn't need to be accurate, but more accurate upper bound we set, faster convergence rates we get.
    gamma = [gamma_X, gamma_Y] (optional): The vector of smoothness for X and Y. 
    ...
    epsilon = [eps_X, eps_Y] (optional): The vector of bandwidths for X and Y.
                                         If no epsilon is set, automatic bandwidths according to KNN distances will be set.
    epsilon_vector (optional): Possible arguments are 'fixed' or 'range'.
                               If 'fixed' is given, all of the bandwidths for the ensemble estimation will be the same, while if 'range' is chosen, 
   		                       the bandwidths will be arithmetically increasing in a range.	 
    eps_range_factor (optional): If epsilon_vector = 'range', then the range of epsilon is [epsilon, epsilon * (1 + epsilon_vector)].
    normalize_epsilon: If it is True, then the bandwidth will be normalized according to the MI estimate. 
    ensemble_estimation: Several options are available:
   		                 'average': The ensemble estimator is the average of the base estimators.
   		                 'optimal_weights': The ensemble estimator is the wighted sum of the base estimators where the weights are computed using an optimization problem.
   		                 * You need to import cvxpy as cvx (install cvxpy if you do not have it).
                         'n': The ensemble estimator is the median of the base estimators.
    L_ensemble: Number of different base estimators used in ensemble estimation. For more accurate estimates you can increase L_ensemble, but runtime increases linearly as well.
    hashing (optional): Possible arguments are 'p-stable' (default) which is a common type of LSH or 'floor' which uses the simple floor function as hashing.
                        For small dimensions, 'floor', and for higher dimensions, 'p-stable' are preferred.
    stochastic: It is stochastic and the hashing is generated using a random seed.
    
    Output:
    I: The estimation of mutual information between X snd Y. 
"""

import cvxpy as cvx # Requires to install CVXPY package, it is also possible to run this code without cvxpy, by using 'average' or 'median' ensemble_estimation
import math
import numpy as np
import sklearn
import time
...
# Compute mutual information and gradient given epsilons and random shifts
