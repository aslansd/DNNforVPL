    ...
    for num_simulation in range(0, 1):
        ...
        for group_training in ['group1', 'group2', 'group3', 'group4']:
            ...
            for layer_freeze in [None, 0, 3]:
                ...
                x_val_training = np.zeros((len(SF_training) * len(Ori_training) * 180, 112, 112, 3), dtype = np.float32)
                ...
                # Define the main variables
                x_val_tuning = np.zeros((len(SF_tuning) * len(Ori_tuning) * 180, 112, 112, 3), dtype = np.float32)
...
#                # The central units of consecutive convolutional layers: (55, 55)
...
#                        all_central_unit_activity_Conv2d_2[i, :] = unit_activity_layer_3[0, :, 55, 55].detach().cpu().clone().numpy()
#                        
...
#                    best_acc1 = max(validation_accuracy[session - start_session], best_acc1)        
                ...
                # Define the main variables
                x_val_transfer = np.zeros((len(SF_transfer) * len(Ori_transfer) * 180, 112, 112, 3), dtype = np.float32)
